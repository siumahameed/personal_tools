...
def get_documents():
    import json, os
    path = os.path.join(BASE, "data/scholar_documents.json")
    if not os.path.exists(path):
        return {"samples": []}
...
def clear_documents():
    ...
    log_path = os.path.join(BASE, "data", "deep_research_log.json")
...
def run_mydoc_generate():
    ...
    try:
        ...
        import json, os
        followed_path = os.path.join(BASE, "data/followed.json")
...
@app.get("/")
def index():
    return FileResponse(os.path.join(BASE, "frontend/dist/index.html"))

# Mount static files (ensure this is registered after all dynamic routes)
app.mount("/", StaticFiles(directory=os.path.join(BASE, "frontend/dist")), name="static")
