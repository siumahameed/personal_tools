      ...
      <div v-for="doc in filteredDocs" :key="doc.id" class="doc-card">
        <div class="doc-badges-row">
          <div class="doc-type-badge" :class="doc.type">
            {{ doc.type === 'sop' ? 'SOP' : doc.type === 'resume' ? 'CV' : 'LOR' }}
          </div>
        ...
        <div class="doc-card-body">
          ...
          <div class="doc-takeaways" v-if="doc.key_takeaways">
...
export default {
  ...
  computed: {
    subTabs() {
      const counts = { sop: 0, resume: 0, recommendation: 0 }
  ...
  methods: {
    ...
    sourceLabel(source) {
      ...
      if (s.includes('Bangladesh') || s.includes('Discord')) return 'BD'
      ...
      if (s.includes('Substack')) return 'Sub'
      if (s.includes('ResearchGate')) return 'RG'
      if (s.includes('Academia')) return 'Acad'
      if (s.includes('Issuu')) return 'Issuu'
      if (s.includes('Scribd')) return 'Scribd'
